...
void rexiIntegrationBasedOnPDE(
		const Tcomplex &i_lambda,	///< stiffness
		T i_dt,					///< timestep size
		const sweet::REXICoefficients<T> &i_rexiCoefficients,
...
)
{
	...
	sweet::ShackExpIntegration *shackExpIntegration = shackProgArgDict.getAutoRegistration<sweet::ShackExpIntegration>();
	sweet::ShackIOData *shackIOData = shackProgArgDict.getAutoRegistration<sweet::ShackIOData>();
	ERROR_CHECK_WITH_PRINT_AND_COND_RETURN_EXIT(shackProgArgDict);
	...
	if (shackExpIntegration->exp_method != "file")
	{
		...
		{
			shackExpIntegration->p_rexi_files_processed.push_back(sweet::ShackExpIntegration::REXIFile("phi0", ""));
		}
		else if (shackExpIntegration->exp_method == "ci")
		{
			...
			for (int i = 0; i < 9; i++)
				shackExpIntegration->p_rexi_files_processed.push_back(sweet::ShackExpIntegration::REXIFile(function_names[i], ""));
		}
		else
		{
	...
	for (std::size_t i = 0; i < shackExpIntegration->p_rexi_files_processed.size(); i++)
	{
		...
		sweet::REXICoefficients<T> rexiCoefficients;
		sweet::REXI<T>::load(
				shackExpIntegration,
				function_name,
				rexiCoefficients,
