...
/*Debug output*/
	...
	)
	{
		...
		T f = shackSWEPlane->plane_rotating_f0;
		T h = shackSWEPlane->h0;
		T g = shackSWEPlane->gravitation;
		...
		if (shackSWEPlane->plane_rotating_f0 == 0)
		{
			/*
	...
	)
	{
		...
		if (shackNormalModeAnalysis->normal_mode_analysis_generation == 4)
		{
			...
			file << "# g " << shackSWEPlane->gravitation << std::endl;
			file << "# h " << shackSWEPlane->h0 << std::endl;
		...
		else
		{
			...
			if (shackPlaneDataOps->timestepping_method == SimulationVariables::Discretization::LEAPFROG_EXPLICIT)
			{
				...
				shackTimestepControl->current_timestep_size = 0.5*shackSWEPlane->CFL;
				shackSWEPlane->CFL = -shackTimestepControl->current_timestep_size;
			}
#endif
			...
			file << "# h " << shackSWEPlane->h0 << std::endl;
//			file << "# r " << shackSWEPlane->sphere_radius << std::endl;
			file << "# f " << shackSWEPlane->plane_rotating_f0 << std::endl;
