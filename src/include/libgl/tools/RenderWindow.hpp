...
//				printf(" + line: %i\n", line);
	...
	bool setupOpenGLRenderContext(
			const char* i_initial_window_title,
...
public:
	 ...
	 */
	RenderWindow(
			const char *i_initial_window_title,					///< title of window to display in the window title bar
	 ...
	 */
	void setWindowTitle(
			const char *i_title	// string of title
	)
	{
		SDL_SetWindowTitle(window, i_title);
	}
