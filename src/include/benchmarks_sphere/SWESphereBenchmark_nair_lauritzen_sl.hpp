...
#ifndef SRC_SWE_SPHERE_BENCHMARKS_NAIR_LAURITZEN_SL_HPP_
#define SRC_SWE_SPHERE_BENCHMARKS_NAIR_LAURITZEN_SL_HPP_
...
class SWESphereBenchmark_nair_lauritzen_sl	: public SWESphereBenchmarks_interface
{
...
public:
	SWESphereBenchmark_nair_lauritzen_sl()
	{
	}
...
*/
	...
	)	const
	{
		...
		if (benchmark_name == "nair_lauritzen_case_1")
		{
			...
			SphereData_Spectral stream = ops->scalar_physical_to_spectral(stream_phys);
			stream *= u0;

			SphereData_DebugContainer::append(ops->laplace(stream), "vort_from_stream");
			SphereData_DebugContainer::append(o_vrt, "vort");

			o_vrt = ops->laplace(stream);
