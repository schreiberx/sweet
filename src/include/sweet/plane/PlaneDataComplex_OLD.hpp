...
#		pragma omp parallel for OPENMP_PAR_SIMD
	...
	inline
	void p_physical_set(int y, int x, double i_real, double i_imag)
	{
		data[(y*planeDataConfig->physical_res[0]+x)*2+0] = i_real;
		data[(y*planeDataConfig->physical_res[0]+x)*2+1] = i_imag;
	}
