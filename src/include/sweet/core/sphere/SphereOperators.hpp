	...
	)
	{
		...
		if (i_shackSphereDataOps->sphere_use_fsphere)
		{
			fg.physical_update_lambda_gaussian_grid(
				[&](double lon, double mu, double &o_data)
				{
					o_data = i_shackSphereDataOps->sphere_fsphere_f0;
				}
			);
		}
		else
		{
			fg.physical_update_lambda_gaussian_grid(
				[&](double lon, double mu, double &o_data)
				{
					o_data = mu*2.0*i_shackSphereDataOps->sphere_rotating_coriolis_omega;
				}
			);
		}
		...
		mx.resize(2*sphereDataConfig->shtns->nlm);

		st_dt_matrix(sphereDataConfig->shtns, mx.data());
