...
/*Debug output*/
	...
	)
	{
		...
		else
		{
			...
			sprintf(buffer_real, filename, "normal_modes_physical", shackTimestepControl->current_timestepSize*shackIOData->output_time_scale);
			std::ofstream file(buffer_real, std::ios_base::trunc);
			...
			if (i_shackDict.disc.timestepping_method == sweet::ShackDictionary::Discretization::LEAPFROG_EXPLICIT)
			{
				...
				shackTimestepControl->current_timestepSize = 0.5*i_shackDict.sim.CFL;
				i_shackDict.sim.CFL = -shackTimestepControl->current_timestepSize;
			}
#endif
