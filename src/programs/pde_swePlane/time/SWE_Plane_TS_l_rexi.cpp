...
#	pragma omp parallel for schedule(static,1) default(none) shared(i_dt, i_h_pert, i_u, i_v, max_N, std::cout, std::cerr)
	...
	for (int i = 0; i < num_local_rexi_par_threads; i++)
	{
		...
		sweet::PlaneData_SpectralComplex &v0 = perThreadVars[i]->v0;

		sweet::PlaneData_SpectralComplex &h_sum = perThreadVars[i]->h_sum;
		sweet::PlaneData_SpectralComplex &u_sum = perThreadVars[i]->u_sum;
		sweet::PlaneData_SpectralComplex &v_sum = perThreadVars[i]->v_sum;

		sweet::PlaneData_SpectralComplex &eta = perThreadVars[i]->eta;
		...
		u0 = sweet::Convert_PlaneDataSpectral_To_PlaneDataSpectralComplex::physical_convert(i_u);
		v0 = sweet::Convert_PlaneDataSpectral_To_PlaneDataSpectralComplex::physical_convert(i_v);
#else
...
///		else
///		{
///			eta0 = sweet::Convert_PlaneDataSpectral_To_PlaneDataSpectralComplex::spectral_convert(i_h_pert);
///			u0 = sweet::Convert_PlaneDataSpectral_To_PlaneDataSpectralComplex::spectral_convert(i_u);
///			v0 = sweet::Convert_PlaneDataSpectral_To_PlaneDataSpectralComplex::spectral_convert(i_v);
///		}
#endif
		...
		for (std::size_t n = start; n < end; n++)
		{
			...
			if (shackPDESWEPlane->plane_rotating_f0 == 0)
			{
				...
				sweet::PlaneData_SpectralComplex u1 = (u0 + g*opc.diff_c_x(eta))*(1.0/alpha);
				sweet::PlaneData_SpectralComplex v1 = (v0 + g*opc.diff_c_y(eta))*(1.0/alpha);
			...
			else
			{
				...
				sweet::PlaneData_SpectralComplex lhs = lhs_a.spectral_addScalarAll(kappa);
				eta = rhs.spectral_div_element_wise(lhs);
...
///		perThreadVars[n]->v_sum.request_data_spectral();
		...
		sweet::PlaneData_Spectral tmp(planeDataConfig);

		o_h_pert = o_h_pert + sweet::Convert_PlaneDataSpectralComplex_To_PlaneDataSpectral::spectral_convert_physical_real(perThreadVars[n]->h_sum);
		o_u = o_u + sweet::Convert_PlaneDataSpectralComplex_To_PlaneDataSpectral::spectral_convert_physical_real(perThreadVars[n]->u_sum);
		o_v = o_v + sweet::Convert_PlaneDataSpectralComplex_To_PlaneDataSpectral::spectral_convert_physical_real(perThreadVars[n]->v_sum);
	}
#endif
...
#if !SWEET_USE_PLANE_SPECTRAL_SPACE
	o_h_pert = sweet::Convert_PlaneDataSpectralComplex_To_PlaneDataSpectral::physical_convert(perThreadVars[0]->h_sum);
	o_u = sweet::Convert_PlaneDataSpectralComplex_To_PlaneDataSpectral::physical_convert(perThreadVars[0]->u_sum);
	o_v = sweet::Convert_PlaneDataSpectralComplex_To_PlaneDataSpectral::physical_convert(perThreadVars[0]->v_sum);
...
//		if (shackDict.rexi.use_half_poles)
	if (true)
	{
		...
		o_u = sweet::Convert_PlaneDataSpectralComplex_To_PlaneDataSpectral::physical_convert_real(perThreadVars[0]->u_sum);
		o_v = sweet::Convert_PlaneDataSpectralComplex_To_PlaneDataSpectral::physical_convert_real(perThreadVars[0]->v_sum);
	}
////	else
////	{
////		o_h_pert = sweet::Convert_PlaneDataSpectralComplex_To_PlaneDataSpectral::spectral_convert_physical_real(perThreadVars[0]->h_sum);
////		o_u = sweet::Convert_PlaneDataSpectralComplex_To_PlaneDataSpectral::spectral_convert_physical_real(perThreadVars[0]->u_sum);
////		o_v = sweet::Convert_PlaneDataSpectralComplex_To_PlaneData::spectral_convert_physical_real(perThreadVars[0]->v_sum);
////	}
#endif
	...
	sweet::PlaneData_Spectral tmp(o_h_pert.planeDataConfig);
...
{
#if SWEET_MPI
	sweet::PlaneData_Spectral dummyData(i_planeDataConfig);
