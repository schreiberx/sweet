...
SWE_Sphere_TS_PFASST_l_rexi::SWE_Sphere_TS_PFASST_l_rexi(
...
void SWE_Sphere_TS_PFASST_l_rexi::reset()
{
	#if SWEET_BENCHMARK_TIMINGS
		SimulationBenchmarkTimings::getInstance().rexi.start();
		SimulationBenchmarkTimings::getInstance().rexi_setup.start();
...
void SWE_Sphere_TS_PFASST_l_rexi::p_get_workload_start_end(
		std::size_t &o_start,
		std::size_t &o_end,
...
void SWE_Sphere_TS_PFASST_l_rexi::p_update_coefficients(
		bool i_update_rexi
)
{
	if (i_update_rexi)
	...
	for (int local_thread_id = 0; local_thread_id < num_local_rexi_par_threads; local_thread_id++)
	{
		...
		{
#if 0
			SWEETError("TODO: update me to new implementation");
...
void SWE_Sphere_TS_PFASST_l_rexi::run_timestep_pert(
 ...
 */
void SWE_Sphere_TS_PFASST_l_rexi::run_timestep_nonpert(
...
)
{
	 ...
	 */
		...
		else
		{
			...
			SphereData_Physical prog_phi0_phys = io_prog_phi0.toPhys();
			SphereData_Physical prog_vort0_phys = io_prog_vort0.toPhys();
			SphereData_Physical prog_div0_phys = io_prog_vort0.toPhys();
