...
SWE_Sphere_TS_PFASST_l_cn::SWE_Sphere_TS_PFASST_l_cn(
 ...
 */
void SWE_Sphere_TS_PFASST_l_cn::setup(
...
void SWE_Sphere_TS_PFASST_l_cn::update_coefficients()
{
	if (!use_f_sphere)
	{
...
void SWE_Sphere_TS_PFASST_l_cn::run_timestep_nonpert(
...
)
{
	...
	else
	{
		...
		op.robert_vrtdiv_to_uv(vort0, div0, ug, vg);
		SphereData_Physical phig = phi0.toPhys();
	...
	else
	{
		...
		SphereData_Physical foo =
				(gh*(div0.toPhys() - (1.0/alpha)*two_coriolis*mug*vort0.toPhys())) +
