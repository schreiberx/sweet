...
void SWE_Sphere_TS_PFASST_lg_erk_lc_n_t_erk::euler_timestep_update_linear(
...
void SWE_Sphere_TS_PFASST_lg_erk_lc_n_t_erk::euler_timestep_update_coriolis_and_nonlinear(
		const SphereData_Spectral &i_phi,	///< prognostic variables
		const SphereData_Spectral &i_vort,	///< prognostic variables
		const SphereData_Spectral &i_div,	///< prognostic variables
...
)
{
	...
	// include the topography term
	o_div_t += -op.laplace(phi_topo);
 ...
 */
void SWE_Sphere_TS_PFASST_lg_erk_lc_n_t_erk::euler_timestep_update(
		const SphereData_Spectral &i_phi,	///< prognostic variables
		const SphereData_Spectral &i_vort,	///< prognostic variables
		const SphereData_Spectral &i_div,	///< prognostic variables
...
void SWE_Sphere_TS_PFASST_lg_erk_lc_n_t_erk::run_timestep_nonpert(
		SphereData_Spectral &io_phi,		///< prognostic variables
		SphereData_Spectral &io_vort,	///< prognostic variables
		SphereData_Spectral &io_div,		///< prognostic variables
...
)
{
	if (timestepping_order == 1)
	{
		timestepping_rk_linear.run_timestep(
				this,
				&SWE_Sphere_TS_PFASST_lg_erk_lc_n_t_erk::euler_timestep_update_linear,	///< pointer to function to compute euler time step updates
		...
		timestepping_rk_nonlinear.run_timestep(
				this,
				&SWE_Sphere_TS_PFASST_lg_erk_lc_n_t_erk::euler_timestep_update_coriolis_and_nonlinear,	///< pointer to function to compute euler time step updates
				io_phi, io_vort, io_div,
				i_dt,
				timestepping_order,
				i_simulation_timestamp
 ...
 * Setup
 */
void SWE_Sphere_TS_PFASST_lg_erk_lc_n_t_erk::setup(
...
)
{
	...
	SphereData_Spectral tmp(simVars.benchmark.h_topo);

	// restrict the topography to this space-time level
	phi_topo = tmp.spectral_returnWithDifferentModes(phi_topo.sphereDataConfig);
...
SWE_Sphere_TS_PFASST_lg_erk_lc_n_t_erk::SWE_Sphere_TS_PFASST_lg_erk_lc_n_t_erk(
		...
		phi_topo(i_op.sphereDataConfig)
{
