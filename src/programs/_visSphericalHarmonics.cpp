...
#ifndef SWEET_GUI
	#error "Only makes sense with GUI enabled"
#endif
...
class SimulationVisualizationInstance
{
public:
	...
	sweet::SphereData_Spectral *prog_phi_pert = nullptr;
	sweet::SphereData_Spectral *prog_vrt = nullptr;
	sweet::SphereData_Spectral *prog_div = nullptr;

#if SWEET_GUI
	sweet::PlaneData_Physical *viz_plane_data = nullptr;
#endif
...
public:
	...
	void setup_mode()
	{
		...
		sweet::SphereData_Spectral a = *prog_phi_pert;

		SphereData_DebugContainer::append(a, "a");

		sweet::SphereData_Spectral mu_a = ops->mu(a);
		SphereData_DebugContainer::append(mu_a, "mu(a)");

		sweet::SphereData_Spectral mug_a = ops->mug*a.toPhys();
		SphereData_DebugContainer::append(mug_a, "mug*a");

		sweet::SphereData_Spectral diff = mu_a - mug_a;
	...
	void reset(bool online_reset = false)
	{
		...
		if (online_reset)
		{
			shackDict.disc.space_res_physical[0] = 0;
			shackDict.disc.space_res_physical[1] = 0;
		...
		ops = new SphereOperators(sphereDataConfig, &(shackDict.sim));
		...
		if (shackDict.benchmark.benchmark_name == "m")
		{
			shackDict.misc.vis_id = -1;
		...
		else if (shackDict.benchmark.benchmark_name == "a")
		{
			SphereData_DebugContainer::clear();

			sweet::SphereData_Spectral one(sphereDataConfig);
			...
			sweet::SphereData_Spectral mu_one = ops->mu(one);
			SphereData_DebugContainer::append(mu_one, "mu(one)");

			sweet::SphereData_Spectral mug_one = ops->mug*one.toPhys();
			SphereData_DebugContainer::append(mug_one, "mug*one");

			sweet::SphereData_Spectral diff = mu_one - mug_one;
			SphereData_DebugContainer::append(diff, "diff");
		}
		else if (shackDict.benchmark.benchmark_name == "b")
		{
			SphereData_DebugContainer::clear();

			sweet::SphereData_Spectral mu = ops->mug;

			SphereData_DebugContainer::append(mu, "mu");
			SphereData_DebugContainer::append(ops->mug, "mug");

			sweet::SphereData_Spectral mu_mu = ops->mu(mu);
			SphereData_DebugContainer::append(mu_mu, "mu(mu)");

			sweet::SphereData_Spectral mug_mu = ops->mug*mu.toPhys();
			SphereData_DebugContainer::append(mug_mu, "mug*mu");

			sweet::SphereData_Spectral diff = mu_mu - mug_mu;
			SphereData_DebugContainer::append(diff, "diff");
		}
		else
		{
			BenchmarksSphereSWE sphereBenchmarks;
			sphereBenchmarks.setup(shackDict, *ops);
...
public:
	...
	)
	{
		...
		*o_bogus_data = sphereDataConfig;
		...
		switch (id)
		{
			...
			case 3:
				*viz_plane_data = Convert_SphereDataSpectral_To_PlaneDataPhysical::physical_convert(shackDict.sim.h0 + SphereData_Spectral(*prog_phi_pert)/shackDict.sim.gravitation, planeDataConfig);
				break;
			...
			case 8:
			{
				...
				sweet::SphereData_Spectral anal_solution_vort(sphereDataConfig);
	...
	void vis_keypress(int i_key)
	{
		...
		case 't':
			shackDict.disc.space_res_spectral[0] *= 2;
...
{
	if (!shackDict.setupFromMainParameters(i_argc, i_argv, nullptr))
		return -1;
...

	{
		...
		{
			SimulationVisualizationInstance *sim = new SimulationVisualizationInstance;
			VisSweet<SimulationVisualizationInstance> visSweet(sim);
			delete sim;
		}
		else
#endif
