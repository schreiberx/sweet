...
public:
	...
	bool runTimestep()
	{
		...
		if (shackIOData->verbosity > 2)
			if (shackParallelization->isMPIRoot)
			{
				double output_time = shackTimestepControl->current_simulation_time*shackTimestepControl->output_simulation_time_scalar;
				std::cout << shackTimestepControl->current_timestep_nr << ": " << output_time << std::endl;
			}
