...
public:
	...
	void write_output()
	{
		...
		SphereData vort = op.vort(prog_u, prog_v)/simVars.earth_radius;
		if (simVars.benchmark_scenario_id == 0)
...
//				setup_initial_conditions_gaussian(-M_PI/3.0);
		...
		SphereDataConfig sphConfigRexiAddedModes;

		// Pointer to SPH configuration for REXI computations
		SphereDataConfig *sphConfigRexi = nullptr;
		...
		else
		{
			...
			SphereData tmp_prog_phi(sphConfigRexi);
			SphereData tmp_prog_u(sphConfigRexi);
			SphereData tmp_prog_v(sphConfigRexi);

			SphereData accum_prog_phi(sphConfigRexi);
			SphereData accum_prog_u(sphConfigRexi);
			SphereData accum_prog_v(sphConfigRexi);
			...
			while (simVars.timecontrol.current_simulation_time < simVars.timecontrol.max_simulation_time)
			{
...

				{
					...
					SphereData prog_phi_rexi(sphConfigRexi);
					SphereData prog_u_rexi(sphConfigRexi);
					SphereData prog_v_rexi(sphConfigRexi);
