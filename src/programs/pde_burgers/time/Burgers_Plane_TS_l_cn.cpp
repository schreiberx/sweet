...
)
{
	...
	sweet::PlaneData_Spectral rhs_u = io_u + 0.5*dt*simVars.sim.viscosity*(op.diff2_c_x(io_u) + op.diff2_c_y(io_u));
	sweet::PlaneData_Spectral rhs_v = io_v + 0.5*dt*simVars.sim.viscosity*(op.diff2_c_x(io_v) + op.diff2_c_y(io_v));
	sweet::PlaneData_Spectral lhs = ((-0.5*dt)*simVars.sim.viscosity*(op.diff2_c_x + op.diff2_c_y)).spectral_addScalarAll(1.0);
