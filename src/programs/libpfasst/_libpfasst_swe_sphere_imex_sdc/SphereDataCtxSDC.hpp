...
public:
        ...
        : shackDict(i_shackDict), levelSingleton(i_singleton)
    {
        ...
        timestepper_lg_erk_lc_n_erk = new PDESWESphereTS_lg_erk_lc_n_erk(*shackDict, levelSingleton->ops);
        timestepper_lg_irk = new PDESWESphereTS_lg_irk(*shackDict, levelSingleton->ops);
        ...
        timestepper_lg_irk->setup(timestepping_order, shackTimestepControl->current_timestep_size);	

        // initialize the residuals
        residuals.resize(nprocs,std::vector<double>(0,0.));

        // initialize the diagnostics object
        sphereDiagnostics = new PDESWESphere_Diagnostics(
...
protected:
    ...
    PDESWESphere_Diagnostics* sphereDiagnostics;
