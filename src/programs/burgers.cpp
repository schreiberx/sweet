...
#include <sweet/plane/PlaneData_Physical_Spectral.hpp>
...
////			tmp_u.request_data_spectral();
		...
		if (simVars.misc.verbosity > 0)
		{
			...
			if (simVars.misc.compute_errors)
			{
				...
				tmp = compute_errors2(u_phys, v_phys);
...
public:
	...
	)
	{
		...
		if (simVars.misc.compute_errors)
		{
			...
			benchmark.benchmark_analytical_error_rms_u = (ts_u-u).physical_reduce_rms_quad();
			benchmark.benchmark_analytical_error_rms_v = (ts_v-v).physical_reduce_rms_quad();

			benchmark.benchmark_analytical_error_maxabs_u = (ts_u-u).physical_reduce_max_abs();
			benchmark.benchmark_analytical_error_maxabs_v = (ts_v-v).physical_reduce_max_abs();
	...
	bool instability_detected()
	{
		...
		return !(	u.toPhys().reduce_boolean_all_finite() &&
					v.toPhys().reduce_boolean_all_finite()
				);
	}
