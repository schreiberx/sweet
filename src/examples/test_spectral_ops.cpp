...
//	std::size_t res_x = 256;
	{
		...
		DataArray<2> two(res);
		DataArray<2> constant(res);

		zero.setAll(0);
		two.setAll(2);
		constant.setAll(5);

//		std::cout << (constant-(two+3)).reduce_sumAbs_quad() << std::endl;
//		std::cout << ((two+1)).reduce_sumAbs_quad()/(double)(res[0]*res[1]) << std::endl;
//		continue;
...
#else
		double err_x = (op.diff_c_x(h)-h_diff_x).reduce_maxAbs();
		double err_y = (op.diff_c_y(h)-h_diff_y).reduce_maxAbs();
		double err_z = (u*v-h).reduce_maxAbs();
#endif

//		h_diff_x.requestDataInSpectralSpace();
//		h_diff_x.array_data_cartesian_space_valid = false;
//		std::cout << op.diff_c_x(h).reduce_sumAbs_quad() << "\t" << h_diff_x.reduce_sumAbs_quad() << std::endl;
//		std::cout << (op.diff_c_x(h)+h_diff_x).reduce_sumAbs_quad() << "\t" << (h-h_diff_x).reduce_sumAbs_quad() << std::endl;

		if (parameters.use_spectral_diffs)
		{
			std::cout << res_x << "\t" << res_y << "\t" << err_x << "\t" << err_y << "\t" << err_z << std::endl;
		...
		{
			std::cout << res_x << "\t" << res_y << "\t" << err_x << "\t" << err_y << "\t" << err_z << "\t" << prev_error_x/err_x << "\t" << prev_error_y/err_y  << "\t" << prev_error_z/err_z << std::endl;
