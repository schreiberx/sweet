...
subprocess.call('scons --program=swe_rexi --spectral-space=enable --spectral-dealiasing=disable --threading=off --rexi-parallel-sum=enable --mode=release '.split(' '), shell=False)
...
for n in N_list:
	...
	filename = curdir_name+'/'+output_file_prefix+"_n"+str(n)+"_eps"+str(eps)+"_dt"+str(dt)+".csv"
	print "Writing output to "+filename
	fd = open(filename, "w")

	fd.write("#TI res="+str(n)+"x"+str(n)+", eps="+str(eps)+", dt="+str(dt)+", DT="+str(max_time)+", "+scenario_name+"\n")
	fd.write("#TX REXI parameter M\n")
	fd.write("#TY REXI parameter h\n")
	fd.write("h\M")
	...
	for h in h_list:
		...
		for M in M_list:
			...
			command += ' -S 0 '
			command += ' --use-specdiff-for-complex-array 0 '
