...
echo "*** GCC7.2 ***"
if [ ! -e "$SWEET_LOCAL_SOFTWARE_DST_DIR/bin/gcc-7.2"  -o "$1" != "" ]; then
	...
	export CXX=g++
	export LINK=ld
	./configure --disable-multilib --enable-languages=c++,fortran  --prefix="$SWEET_LOCAL_SOFTWARE_DST_DIR" --program-suffix=-7.2 || exit 1

	make || exit 1
	make install || exit 1

	for i in g++ gcc gcc-ar gcc-nm gcc-ranlib gfortran gcov gcov-tool gfortran; do
		ln -sf "$SWEET_LOCAL_SOFTWARE_DST_DIR/bin/$i-7.2" "$DST_DIR/bin/$i"
	done
