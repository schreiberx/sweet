...
$e^{tL}$ we assume that $L$ is skew hermitian and therefore has
only purely imaginary eigenvalues, and maybe decomposed as $\Sigma\Lambda\Sigma^{H}$,
yielding 
\[
e^{tL}=\sum_{k=0}^{\infty}\frac{tL}{k!}=\Sigma\left(\sum_{k=0}^{\infty}\frac{t\Lambda}{k!}\right)\Sigma^{H}=\Sigma e^{t\Lambda}\Sigma^{H},
\]
where we used the orthonormality of $\Sigma$ to remove it from the
summation, and
...
therefore $\lambda_{n}$ are assumed real. Since $e^{t\Lambda}$ is
diagonal, it can be eigenvalue-wise approximated in the same way as
in $e^{ix}$.
...
We want to evaluate $e^{\tau L}U(0)$ with REXI, where $\tau$ will
be a time step size and $U(0)$ the initial condition for this time
step. We will assume $\tau$ a-priori fixed, which implies that the
coefficients in REXI will not change and may be pre-computed.
...
by equation \eqref{eq:alpha}. These coefficient may be pre-computed
if $L$ and $\tau$ are fixed.

Note, that for debugging purpose, their \emph{imaginary values have
to cancel out}.\textbf{ (TODO - PP: are you sure? MaS: No, but this
is what Terry mentioned).}

Note an important property (see Sec. 3.3 in \cite{Terry:High-order time-parallel approximation of evolution operators}).
...
\textbf{@Pedro: I\textasciiacute ve now also uploaded the jpeg ( IMG\_20150806\_002902.jpg
) which I\textasciiacute ve sent you 10 days ago.}\\
\textbf{This is how it is currently implemented and knowing the problems
in this formulations would greatly simplify debugging. Maybe you can
rearrange these equations in a form which match closely this formulations
and check where the problem in this formulation (see jpeg) is?}\\
\textbf{@Pedro: The equations above were also derived with (L+\textbackslash{}alpha),
not with (L-\textbackslash{}alpha). Sorry, but I forgot to update
also the rest of this document with (L+\textbackslash{}alpha).}

\textbf{@Martin: Why not stick with Terry's paper standard?}\\
\textbf{@Pedro: $\delta$ is already used above as the Kronecker delta
and the derivative. Maybe we should avoid overutilising it :-).}

@Martin: We should not be using \$\textbackslash{}delta\$ for derivative
(where are we doing this?)\\
\textbf{@Pedro: Consistency with formulations of SWE: $H=\eta_{0}$,
$\eta_{0}=\eta(0)$, $\eta_{1}=\eta(1)$, $u_{0}=u(0)$, $v_{0}=v(0)$... }

\textbf{@ Martin: I used \$\textbackslash{}bar\{\textbackslash{}eta\}\$
instead of \$\textbackslash{}eta\_0\$ for the mean fluid depth - needs
to be changed in formulation doc}\\
...
(L+\alpha)U=U(0)
\]
so that $U=(L+\alpha)^{-1}U_{0}$. We will do this converting the
problem into and elliptic equation.\textbf{}\\
...
reads $H\delta-\alpha\eta=\eta_{0}$, results in 
\[
...
\begin{equation}
\Delta\eta-\kappa^{2}\eta=r_{0}\label{eq:helmhotz}
\end{equation}
where 
\[
...
Multiplying the equation \eqref{eq:helmhotz} by $-g\bar{\eta}$ gives 

\[
((\alpha^{2}+f^{2})-g\bar{\eta}\Delta)\eta-\kappa^{2}\eta=\frac{f^{2}+\alpha^{2}}{\alpha}\eta_{0}-\eta\delta_{0}-\frac{f\bar{\eta}}{\alpha}\zeta_{0}
\]


Once $\eta$ is calculated from any Helmholtz solver, we need to retrieve
the velocities by solving the $2\times2$ system formed by equations
\eqref{eq:mom_u} and \eqref{eq:mom_v}, which reads 
\[
A_{\alpha}U=U_{0}+g\nabla\eta
\]
with 
...
The solution is 
\begin{equation}
u=-\frac{1}{f^{2}+\alpha^{2}}\left[g(\alpha\eta_{x}-f\eta_{y})+\alpha u_{0}-fv_{0}\right]\label{eq:elliptic_velocity_u}
\end{equation}
and 
\begin{equation}
v=-\frac{1}{f^{2}+\alpha^{2}}\left[g(f\eta_{x}+\alpha\eta_{y})+fu_{0}+\alpha v_{0}\right],\label{eq:elliptic_velocity_v}
\end{equation}
...
\textbf{@Pedro: (L+\textbackslash{}alpha) with a plus sign ...}
...
\[
(D-i\alpha^{i})(U^{r}+iU^{i})=U_{0}^{r}+iU_{0}^{i},
\]
\[
...
then solver for the velocity in Eqs. (\ref{eq:elliptic_velocity_u},\ref{eq:elliptic_velocity_v}).
...
difference/element methods, the problems needs to be split into its
real and imaginary parts.

Then, the problem is reduced to computing the REXI as given in Eq.
\eqref{eq:rexi}. We like to note again, that the $\alpha_{n}$ and
$\beta_{n}$ are independent of the system $L$ to solve, and the
number of coefficients only depends on the accuracy and the resolution.
