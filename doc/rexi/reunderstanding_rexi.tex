...
\]
where the $\hat{\cdot}$ symbols indicate the Fourier transforms of the respective functions. The $b_m$ are now the Fourier coefficients of the series for the function $\frac{\hat{f}(\xi)}{\hat{\psi_{h}}(\xi)}$ and be calculated as \footnote{see \cite{High-order time-parallel approximation of evolution operators},
page 11}, 
\[
...
%\]
where we used that $\intop_{-\infty}^{\infty}e^{-\left(\frac{x}{2h}\right)^{2}}dx=h\,\sqrt{4\pi}$. For the case $\xi=\frac{1}{2\pi}$, we get
...
We then combine the approximation (B) of the approximation (A), yielding the approximation for $f$ given by
\[
...
\end{eqnarray*}
By setting the real residue as 
\begin{equation}
...
For the imaginary part of $f(x)$ we analogously have that
\[
...
\section{REXI on linear operators, ``our little wild dog''}
...
\subsection{Solving as an elliptic problem}

We wish to solve the differential problem for each time step
\[ (L-\alpha)U=U_0 \]
so that $U=(L-\alpha)^{-1}U_0$. Expanding the equations with the definition of $L$ brings us to
\begin{eqnarray}
...
\[
\left(1+\frac{\alpha^2}{f^2}\right)\delta =- \frac{\alpha}{f^2}(g \Delta \eta + \delta _0)  -\frac{1}{f}\zeta_0,
\]
\[
...
\label{eq:hemholtz}
\end{equation}
where \[ \kappa^2=\frac{f^2+\alpha^2}{Hg} \] and \[r_0=\eta_0 -\frac{1}{g}\delta_0 - \frac{f}{\alpha g} \zeta_0. \]


Once $\eta$ is calculated, we need to retrieve the velocities by solving the $2\times 2$ system formed by equations (\ref{eq:mom_u}) and (\ref{eq:mom_v}), which gives
\begin{equation}
u=\frac{f}{f^2+\alpha^2}\left[g(\eta_y-\frac{\alpha}{f}\eta_x)+v_0-\frac{\alpha}{f}u_0 \right]
\label{eq:elliptic_velocity_u}
\end{equation}
and
\begin{equation}
v=\frac{-\alpha}{f^2+\alpha^2}\left[g(\eta_y+\frac{f}{\alpha}\eta_x)+v_0-\frac{f}{\alpha}u_0 \right].
