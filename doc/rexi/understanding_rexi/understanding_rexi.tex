...
basis function. The reformulation from the discrete sum over this
formulation can be understood by a Riemann integral over one of the
intervals. Therefore, we also don't require the translated Gaussian
since the integrated interval is already shifted to the origin.
...
and completed squares in the exponential term. For the Gaussian function
including a shift $\psi_{h}(x+hm)$, we get
\begin{eqnarray*}
...
the denomiator $ix+h(\mu+i(m+k))$ in terms of $n:=m+k$ by
...
\subsection{Joining real and imaginary exp-approximation}

With the previous formulation, we would have to extract the real components
from the complex values. However, this is not applicable if we use
this for a linear operator. Therefore we require the system to cancel
...
\end{eqnarray*}
Where we used $\beta_{n}^{Re}=\overline{\beta_{-n}^{Re}}$ and $\beta_{n}^{Im}=-\overline{\beta_{-n}^{Im}}$.
Finally, we gain new $\beta$ and $\alpha$ coefficients which don't
require application of the $Re()$ operator. However, one drawback
is that the number of coefficients is enlarged by a factor of two.

\subsection{Example coefficients}
...
\begin{tabular}{|c|c|c|}
\hline 
$n$ & {\scriptsize{}$\alpha_{n}$} & {\scriptsize{}$\beta_{n}^{Re}$}\tabularnewline
\hline 
\hline 
...
for the $\alpha_{n}$ and $\beta_{n}$:

There is an anti-symmetry around the central pole with
\begin{equation}
...
\end{equation}
For sake of simplicity, we only consider the first or second half
of the merged $\alpha$ and $\beta$ from the previous section! In
particular, with $Im(\alpha_{0})=Im(\beta_{0})=0$, there is a zero
imaginary number for the central pole.
...
the approximation
\begin{eqnarray*}
e^{\tau L} & \approx & \sum_{n=-N}^{N}Re\left(\frac{\beta_{n}}{\tau L+\alpha_{n}}\right)\\
 & = & \sum_{n=-N}^{-1}Re\left(\frac{\beta_{n}}{\tau L+\alpha_{n}}\right)+Re\left(\frac{\beta_{0}}{\tau L+\alpha_{0}}\right)+\sum_{n=-N}^{-1}Re\left(\frac{\beta_{N+n+1}}{\tau L+\alpha_{N+n+1}}\right)
\end{eqnarray*}
and using the properties \eqref{eq:alpha_conjugate_symmetry} and
\eqref{eq:beta_conjugate_symmetry}, we can write this as
\begin{eqnarray*}
 &  & \sum_{n=-N}^{-1}Re\left(\frac{\beta_{n}}{\tau L+\alpha_{n}}\right)+Re\left(\frac{\beta_{0}}{\tau L+\alpha_{0}}\right)+\sum_{n=-N}^{-1}Re\left(\frac{\overline{\beta_{N}}}{\tau L+\overline{\alpha_{N}}}\right)\\
 & = & \sum_{n=-N}^{-1}\left(Re\left(\frac{\beta_{n}}{\tau L+\alpha_{n}}+\overline{\left(\frac{\beta_{n}}{\tau L+\alpha_{n}}\right)}\right)\right)+Re\left(\frac{\beta_{0}}{\tau L+\alpha_{0}}\right).
\end{eqnarray*}
...
\[
e^{\tau L}\approx\sum_{n=-N}^{N}Re\left(\frac{\beta_{n}}{\tau L+\alpha_{n}}\right)=\sum_{n=-N}^{-1}\left(2\frac{\beta_{n}}{\tau L+\alpha_{n}}\right)+\frac{\beta_{0}}{\tau L+\alpha_{0}}.
\]
...
\subsection{Handling $\tau$ in REXI\label{subsec:Handling-tau-in-REXI}}
...
\]
So instead of solving for $U(\tau)$, we are solving for $U^{\tau}(0):=U(0)\tau^{-1}$
...
We can compute$\eta$ with a solver for elliptic problems. Special
...
\subsection{Errors and fault tolerance}

\textbf{TODO: CHECK THIS, IT MIGHT BE WRONG}

One question arising is what the error is in case that one term of
the REXI approximation is not computed. We assume that this will depend
on $\alpha_{i}$ and continue investigating this further

Directly analysing the errors with the formulation $\left(L-\alpha\right)^{-1}U=U_{0}$
seems to be relatively challenging and we therefore decide to start
direclty with the Helmholtz formulation
\[
((\alpha^{2}+f^{2})-g\bar{\eta}\Delta)\eta=\frac{f^{2}+\alpha^{2}}{\alpha}\eta_{0}-\bar{\eta}\delta_{0}-\frac{f\bar{\eta}}{\alpha}\zeta_{0}
\]
and use the spectral space with selected frequencies $\eta=exp(ikx)$
for all quantities.
...
\]
Considering the tabulated alpha values, we observe a constant real
alpha value and increasing imaginary parts for larger $N$. Hence,
also the denominator increases and results in relatively small solutions
$\hat{\eta}.$ The same accounts for the explicit formulation to compute
the velocities.

We tested this imperically with the code in helmholtz\_problem\_fault\_tolerance.py.

Additionally, we can also observe significantly decreasing $\beta_{i}$
for increasing $i$.

To conclude, the amplitude (the real part) decreases significantly
with increasing $\alpha_{i}$ values. \textbf{TODO: CHECK THIS, IT
MIGHT BE WRONG}
...
the Helmholtz problem for the height and then solver for the velocity.
