...
def constant_recon(dt, ntrunc=0):
...
for  i, ax in enumerate(axes.reshape(-1)):
    dt=dtlist[i]
    quad_points, error_circ, error_ellip = constant_recon(dt, ntrunc=ntrunc)
...
if ntrunc > 0:
    plt.savefig("const_recon_trunc"+str(ntrunc)+".png")
elif ntrunc < 0:
    plt.savefig("const_recon_truncN.png")
else:
    plt.savefig("const_recon.png")
