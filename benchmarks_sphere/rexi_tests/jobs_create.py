...
"""
	...
	def create_job_id(self):
		...
		idstr += '_C'+str(self.timestep_size).ljust(8, '0')
		idstr += '_t'+str(self.simtime).ljust(8, '0')
		idstr += '_o'+str(self.output_timestep_size).ljust(8, '0')
...
if True:
	...
	for p.rexi_m in [512]:
		for p.rexi_extended_modes in [0, 2, 8, 32]:
			p.gen_script('script'+p.create_job_id(), 'run.sh')
