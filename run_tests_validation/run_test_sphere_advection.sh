...
make clean
#SCONS="scons --threading=omp --unit-test=sphere_advection --gui=disable --plane-spectral-space=disable --sphere-spectral-space=enable --mode=debug"
...
for a in 0 1.4 -0.7; do
	EXEC="./build/test_sphere_advection* -M 64 --dt=$((60*60*1)) -t $((60*60*24*12)) --benchmark=adv_gauss_bump --timestepping-order=2 --timestepping-method=na_sl --advection-rotation-angle=$a"
	echo "$EXEC"
	$EXEC || exit
done
