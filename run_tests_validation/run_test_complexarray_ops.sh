...
if true; then
	...
	EXEC="./build/test_complexarray_ops_spectral_libfft_omp_gnu_release  -X $X -Y $Y -S 0"
	echo "$EXEC"
	$EXEC || exit
	EXEC="./build/test_complexarray_ops_spectral_libfft_omp_gnu_release  -X $X -Y $Y -S 1"
	echo "$EXEC"
	$EXEC || exit
fi
...
make clean
scons --unit-test=test_complexarray_ops --gui=disable --spectral-space=enable --mode=release --spectral-dealiasing=disable
EXEC="./build/test_complexarray_ops_spectral_libfft_omp_gnu_release -n 128 -m 128 -X $X -Y $X -S 1"
echo "$EXEC"
...
make clean
scons --unit-test=test_complexarray_ops --gui=disable --spectral-space=enable --mode=release --spectral-dealiasing=disable
./build/test_complexarray_ops_spectral_libfft_omp_gnu_release -n 128 -m 128 -X $X -Y $X -S 1 || exit
