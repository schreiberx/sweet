...
if true; then
	...
	echo "$EXEC"
	$EXEC || exit
	EXEC="./build/test_complexarray_ops_planespectral_libfft_omp_gnu_release  -X $X -Y $Y -S 1 --use-specdiff-for-complex-array=1"
	echo "$EXEC"
	$EXEC || exit
fi
...
make clean
scons --threading=omp --unit-test=test_complexarray_ops --gui=disable --plane-spectral-space=enable --mode=release --plane-spectral-dealiasing=disable
EXEC="./build/test_complexarray_ops_planespectral_libfft_omp_gnu_release -n 128 -m 128 -X $X -Y $X -S 1 --use-specdiff-for-complex-array=1"
echo "$EXEC"
...
make clean
scons --threading=omp --unit-test=test_complexarray_ops --gui=disable --plane-spectral-space=enable --mode=release --plane-spectral-dealiasing=disable
./build/test_complexarray_ops_planespectral_libfft_omp_gnu_release -n 128 -m 128 -X $X -Y $X -S 1 --use-specdiff-for-complex-array=1 || exit
