...
class sweet_swe_rexi_plane_and_sphere_params:
	target_machine = ''
...
"""
		if self.plane_or_sphere == 'plane':
			default_params = ' --program=swe_plane_rexi --plane-spectral-space=enable --sphere-spectral-space=disable'
		elif self.plane_or_sphere == 'sphere':
			default_params = ' --program=swe_sphere_rexi --plane-spectral-space=disable --sphere-spectral-space=enable'
		else:
			raise Exception("plane or sphere")

		if self.target_machine == '':
			content += """
SCONS="scons --gui=disable """+default_params+""" --mode=release """+"--threading="+ ('omp' if not self.rexi_par else 'off') +" --rexi-thread-parallel-sum=" +('enable' if self.rexi_par else 'disable')+' -j 4"'+"""
echo "$SCONS"
$SCONS || exit 1
...
"""
		...
		if self.rexi_par:
			...
			if self.plane_or_sphere == 'plane':
				...
				content += 'EXEC="$SWEETROOT/build/swe_sphere_rexi_spherespectral_spheredealiasing_omp_libfft_gnu_release'
			else:
				raise Exception("TODO")
		...
		if self.rexi_method == 'terry':
			...
			content += ' --rexi-ci-n='+str(self.rexi_ci_n)
			content += ' --rexi-ci-sx='+str(self.rexi_ci_sx)
			content += ' --rexi-ci-sy='+str(self.rexi_ci_sy)
			content += ' --rexi-ci-mu='+str(self.rexi_ci_mu)
			content += ' --rexi-ci-primitive='+str(self.rexi_ci_primitive)
