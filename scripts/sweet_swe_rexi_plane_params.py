...
class sweet_swe_rexi_plane_params:
	target_machine = ''
...
"""
		if self.target_machine == '':
			content += """
SCONS="scons --program=swe_plane_rexi --gui=disable --plane-spectral-space=enable --mode=release """+"--threading="+ ('omp' if not self.rexi_par else 'off') +" --rexi-thread-parallel-sum=" +('enable' if self.rexi_par else 'disable')+' -j 4"'+"""
echo "$SCONS"
$SCONS || exit 1
